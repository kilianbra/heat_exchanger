...
if PLOT_AQ_SWEEP_NOT_AFR:
    ...
    for i, A_fr in enumerate(A_fr_values):
        ...
        ax.plot(
            x_plot[validity_mask], y_plot[validity_mask], colors[i] + lines[i], label=rf"$g^2$ = {r_s['g2_hot']:.1e}"
        )
    ...
    if PLOT_ENTROPY_NOT_EUERGY:
        ...
        fig.savefig(
            os.path.join(save_dir, "figure4.tiff"),
            dpi=300,
            facecolor="white",
            format="tiff",
            bbox_inches=None,
        ...
        # Save with exact dimensions (no bbox_inches='tight' which crops)
        fig.savefig(
            os.path.join(save_dir, "figure6.tiff"),
            dpi=300,
            facecolor="white",
            format="tiff",
            bbox_inches=None,
    ...
    for i, A_q in enumerate(A_q_values):
        ...
        ax.plot(
            ...
            colors[i] + lines[i],
            label=rf"$g^2 (A_o/A)^2$ = {new_g2_h[0]:.1e}",
        )
    ...
    if PLOT_ENTROPY_NOT_EUERGY:
        # Save with exact dimensions (no bbox_inches='tight' which crops)
        fig.savefig(
            os.path.join(save_dir, "figure5.tiff"),
            dpi=300,
            facecolor="white",
            format="tiff",
            bbox_inches=None,
        ...
        # Save with exact dimensions (no bbox_inches='tight' which crops)
        fig.savefig(
            os.path.join(save_dir, "figure7.tiff"),
            dpi=300,
            facecolor="white",
            format="tiff",
            bbox_inches=None,
