...
YLABEL_CLASSICAL = (
    r"$\xi = (\Delta \dot{W}_{\mathrm{A,hot}} + \Delta \dot{W}_{\mathrm{A,cold}}) / \dot{Q}_{\mathrm{max}}$"
)
# r"Heat Exchanger Availability Change (normalised by $\dot{Q}_{\mathrm{max}}$)"
# r"$\xi^{\mathrm{C}} = (\Delta \dot{W}_{\mathrm{A,hot}}^{\mathrm{C}} + \Delta \dot{W}_{\mathrm{A,cold}}^{\mathrm{C}} ) / \dot{Q}_{\mathrm{max}}$"
YLABEL_PRACTICAL = ""
# r"$\xi^{\mathrm{P}} = (\Delta \dot{W}_{\mathrm{A,hot}}^{\mathrm{P}} + \Delta \dot{W}_{\mathrm{A,cold}}^{\mathrm{P}} ) / \dot{Q}_{\mathrm{max}}$"
...
def save_breakdown2_figures(data=None, do_print=False, save_dir=None):
    """
    Save Breakdown 2 as two separate figures: final_fig_3a (classical), final_fig_3b (practical).
    ...
    for ext in ["svg", "pdf"]:
        fig_a.savefig(Path(save_dir) / f"final_fig_3a.{ext}", dpi=300, facecolor="white", bbox_inches="tight")
    plt.close(fig_a)
    ...
    for ext in ["svg", "pdf"]:
        fig_b.savefig(Path(save_dir) / f"final_fig_3b.{ext}", dpi=300, facecolor="white", bbox_inches="tight")
    plt.close(fig_b)

    print(f"Saved final_fig_3a.svg, final_fig_3a.pdf, final_fig_3b.svg, final_fig_3b.pdf to {save_dir}")
