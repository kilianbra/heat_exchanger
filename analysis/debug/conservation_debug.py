...
print(f"{'=' * 70}")
print("Sequential application: a_is_in=True, b_is_in=True")
print(f"{'=' * 70}")
...
try:
    for step in range(1, n_steps + 1):
        ...
        if step % 5 == 0 or step == n_steps:
            ...
            print(
                f"Step {step:3d}: M={Mach_current:.2f}, T = {T_current:.2f} K (ΔT = {dT:+.2f} K), p = {p_current:.2e} Pa (Δp = {dp / p_b_initial:.2f} % inlet)"
            )
            if T_current > expected_T_final:
                print("Aborting due to T_current > expected_T_final")
                n_final = step
                break
            elif Mach_current > 0.5:
                print("Aborting due to Mach_current > 0.5")
                n_final = step
                break
    if n_final is not None:
        n_steps = n_final
    print()
    print(f"Final state after {n_steps} steps:")
